...
CO2::CO2(Stream* serial, uint8_t pwmpin, uint8_t type, MeasuringRange range) {
  _serial = serial;
  _pwmpin = pwmpin;
  sPwmPin = pwmpin;
  sRange = range;
...
CO2::CO2(Stream* serial, uint8_t type) {
  _serial = serial;
  _type = type;
}

void CO2::activateAsyncUARTReading() {
 ...
 */
void CO2::setDebug(boolean enable, Stream* console) {
  debug = enable;
  _console = console;
  sConsole = console;
  if (debug) {
    _console->println(F("CO2: debug mode ENABLED"));
  } else {
    _console->println(F("CO2: debug mode DISABLED"));
  }
}

boolean CO2::isPreHeating() {
  if (_isBypassPreheatingCheck) {
    ...
    _console->println(F("CO2::isPreHeating() => UNKNOWN SENSOR"));
    return false;
  }
}
...
boolean CO2::isReady() {
  if (isPreHeating()) {
    ...
    _console->print(F("CO2::isReady() => UNKNOWN SENSOR \""));
...
int CO2::readCO2UART() {
  ...
  if (debug) _console->print(F("CO2: - clearing uart reading buffer "));
  ...
  while (_serial->available() > 0 && (unsigned char)_serial->peek() != 0xFF) {
    if (!skip) {
      _console->print(F("CO2: - skipping unexpected readings:"));
      skip = true;
    }
    _console->print(" ");
    _console->print(_serial->peek(), HEX);
  ...
  if (response[8] != check) {
    _console->println(F("CO2: Checksum not OK!"));
    _console->print(F("CO2: Received: "));
    _console->println(response[8], HEX);
    _console->print(F("CO2: Should be: "));
...
void CO2::setTemperatureOffset(uint8_t offset) { _temperatureOffset = offset; }

void CO2::setBypassCheck(boolean isBypassPreheatingCheck, boolean isBypassResponseTimeCheck) {
  _isBypassPreheatingCheck = isBypassPreheatingCheck;
  _isBypassResponseTimeCheck = isBypassResponseTimeCheck;
}

int CO2::getLastCO2() { return sLastPwmPpm; }

byte CO2::getCheckSum(byte* packet) {
  if (_serial == NULL) {
    if (debug) _console->println(F("-- serial is not configured"));
    return STATUS_SERIAL_NOT_CONFIGURED;
  }
...
void CO2::setAutoCalibrate(boolean b)  // only available for CO2-19B with firmware < 1.6, CO2-19C and CO2 14a
{
...
void CO2::setRange(int range)  // only available for CO2-19B < 1.6 and MH-Z 14a
{
...
void CO2::calibrateSpan(int range)
{
